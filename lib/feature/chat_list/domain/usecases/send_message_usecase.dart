...
@Injectable()
class SendMessageUsecase implements UseCase<void, MessageParams> {
  final ChatListRepo _chatListRepo;

  const SendMessageUsecase({required ChatListRepo chatListRepo})
    : _chatListRepo = chatListRepo;

  @override
  Future<void> call(MessageParams params) async {
    final either = await _chatListRepo.sendMessage(message: params);
